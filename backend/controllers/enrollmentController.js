...
const unenrollCourse = async (req, res) => {
  ...
  // Eğer admin ise ve body'de student_id varsa, istediği öğrenciden silsin
  if (req.user.role === "admin" && req.body && req.body.student_id) {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "Kayıt bulunamadı veya zaten silinmiş." });
    }
